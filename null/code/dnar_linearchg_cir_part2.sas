/*******************************************************************
| Name       : dnar_linearchg_cir_part2.sas
| Purpose    : Code to fit copy increments in reference to sim study 
| SAS Version: 9.4
...
| Notes: CIR
|
| Standard copy increments in reference programmed from scratch 
| using the same basic process as the LSHTM/GSK "five macros".
|
| Adapted in line with Wolbers 2022 work by using CIR means for
| observed post IE data 
|
| Basic Idea:
...
%inc "/hpawrk/tad66240/collaboration/eig_estimation/tpe-sim1/null/code/rbi_tools.sas";
...
proc sql noprint;
  create table ds as
    ...
    from data.&scenario.
    where rdrate = &rdrate. and (2500 lt sim_run le 5000) 
    order by rdrate, sim_run, groupn, subjid, visitn, disc;
quit;


********************************************************************;
*** TRANSPOSE DATA TO WIDE FORMAT FOR IMPUTATION MODEL           ***;
********************************************************************;
...
%ods_off();
proc bglimm data = ds (where = (disc = 0))
            outpost = ps 
            seed = &seed.
            nbi = 1000
            nmc = &nimp.
...
data ds1_cir;
  ...
  *** IMPUTATION MODEL FOR MEAN ***;
  do j = 1 to 5;
    if disctime = 0 then m[j] = tt[2,j] + bt[j]*baseline_var;   *** IF DISCTIME = 0 NO INCREMENT JUST REFERENCE ***;
    else m[j] = (j <= disctime)*tt[groupn,j] + (j > disctime)*( tt[groupn,disctime] - tt[2,disctime] + tt[2,j] ) + bt[j]*baseline_var;
  end;
