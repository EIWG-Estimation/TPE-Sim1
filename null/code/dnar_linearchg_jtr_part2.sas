/*******************************************************************
| Name       : dnar_linearchg_jtr_part2.sas
| Purpose    : Code to fit jump to reference to sim study data 
| SAS Version: 9.4
...
| Notes: JTR
|
| Standard jump to reference programmed from scratch using the 
| same process as the LSHTM/GSK "five macros" outlined below.
|
| Basic Idea:
...
| 2. The creation of the jump to reference means and variances 
...
data ds1_jtr;
  ...
  *** IMPUTATION MODEL FOR MEAN ***;
  do j = 1 to 5;
    m[j] = tt[groupn,j]*(d[j]=0) + tt[2,j]*(d[j]=1) + bt[j]*baseline_var;  *** IF DISC USE REF MEAN ***;
  end;
